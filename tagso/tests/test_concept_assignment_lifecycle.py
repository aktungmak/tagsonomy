"""End-to-end lifecycle tests for concept-to-table assignment.

Flow: create scheme, import concept, register table, assign, verify, unassign, cleanup.
"""
...
def test_concept_assignment_create_and_delete(client):
    """Assign concept to table, verify it exists, unassign, verify it is gone."""
    # 1. Create scheme
    ...
    # 2. Import concept (TTL with concept in scheme)
...
"""
    ...
    table_uri = register_resp.get_json()["uri"]

    # 4. Assign concept to table (same as UI)
    assign_resp = client.post(
        "/catalog/assign_concept",
        json={"table_uri": table_uri, "concept_uri": concept_uri},
        headers={"Content-Type": "application/json"},
    )
    assert assign_resp.status_code == 201

    # 5. Verify assignment exists (direct endpoint)
    list_resp = client.get("/catalog/concept_assignments")
    assert list_resp.status_code == 200
    assignments = list_resp.get_json()["assignments"]
    matching = [
        a
        for a in assignments
        if a["table_uri"] == table_uri and a["concept_uri"] == concept_uri
    ]
    ...
    list_resp_after = client.get("/catalog/concept_assignments")
    ...
    # 8. Cleanup: deregister table, delete concept, delete scheme
    client.delete(
        "/catalog/resource",
        json={"uri": table_uri},
        headers={"Content-Type": "application/json"},
